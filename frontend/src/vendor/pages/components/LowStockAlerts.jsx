...
export default function LowStockAlerts({ refreshTrigger }) {
    ...
    useEffect(() => {
        const fetchLowStock = async () => {
            setLoading(true);
            try {
                // We'll fetch all inventory (or up to 100) and filter the low stock ones
                const res = await fetch(`${API_BASE}/vendor/inventory?limit=100`);
                if (res.ok) {
        ...
        <div className="bg-white border border-[#D5E8DC] rounded-xl overflow-hidden min-h-[150px]">
            ...
            <div className="flex flex-col gap-2 p-3 px-3.5 max-h-[200px] overflow-y-auto">
                {loading ? (
                    ...
                    <div className="text-[11px] text-[#27AE60] text-center p-2">Stock levels are good!</div>
                    ...
                    >
                        <div
                            className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${alert.critical ? 'bg-[#FDEDEC]' : 'bg-[#FDEDEC]'
                                }`}
                        >
                            <svg
                                viewBox="0 0 24 24"
                        ...
                        <div>
                            <div className="text-[11px] font-medium text-[#1a1a1a]">{alert.name}</div>
                            <div className={`text-[10px] ${alert.critical ? 'text-[#922B21]' : 'text-[#C0392B]'}`}>
                                {alert.qty}
                            </div>
                        </div>
                        <button
                            className={`ml-auto py-[3px] px-2.5 rounded-md border bg-transparent text-[10px] cursor-pointer whitespace-nowrap hover:bg-[#FDEDEC] ${alert.critical
                                ? 'border-[#922B21] text-[#922B21]'
